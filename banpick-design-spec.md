...
| D19 | Tournament layer              | **Out of scope, permanently.** Casual friendly play only                                                                                               | Reverses the earlier "keep the door open" advice — see §17                                                                                                                     |
...
| D21 | Tie and match resolution      | `HALF_POINT` only. `ALWAYS_3_ROUNDS`, `stopWhenDecided`, no overtime. **A 1.5–1.5 draw is a legal terminal state**                                     | One rule beats four unproven ones. Cuts `COMPENSATION`, `ROLL_OFF`, `VOID_AND_REPLAY`, and with them the whole `advantageHolder` apparatus                                     |
...
| O6  | **Resolved better than feared, 2026-07-28.** The real roster is **40**, not the ~75 assumed. `P(a named character sits among the opponent's draft)` is `draftCount/40` — **10%** at 4, roughly twice the 5.3% this row was written about. `bring-ban1`'s premise still shifts from reading a draft to knowing an opponent's habits, but far less severely | UI copy was rewritten for the habits framing regardless (Phase 4) and stands — it is the honest question at 10% too. §15's meta-ban hit rate remains the measure of whether the mode earns its place, now against a **10% baseline rather than 5.3%**. Still do not compensate by banning more: a coin flip would take seven |
...
A tied round awards 0.5 to each seat. Three rounds are played, the higher score wins, and **1.5–1.5 is a legal terminal state: the match is a draw.** Ties should be rare enough that a drawn match is a curiosity rather than a problem — §15 measures whether that holds.
...
| Termination             | Every `(scoring, resolution, overtime)` triple must be provably terminating. Under D21 the legal table has one row; the validator exists to keep it that way. Note `HALF_POINT` + `FIRST_TO_2` deadlocks at 1.5–1.5 with an empty pool, which is why the pair is not offered |
...
The roster is a versioned JSON asset (`roster/roster.json`), loaded and validated alongside the mode and snapshotted into the creation event. Nothing reads it at runtime. It holds the **40 released Dice Throne heroes** (Season One, Season Two, Marvel, X-Men, Outcasts, and the standalones), compiled 2026-07-28 — see `roster/README.md` for sources. Vanguard's four are due August 2026 and are not in it. See _On size_ below, because the number has design consequences.
...
**Too large and the ban goes blind.** The roster is 40. Under uniform drafting, the chance a named character sits among an opponent's 4 picks is `4/40 = 10%` — a meta ban whiffs about nine times in ten. That is twice as good as the ~75 this section was written against, and it makes O6 a milder problem than it looked. See O6.
...
**Do not compensate with more bans.** At 40, two bans move the hit rate from 10% to about 19%; reaching even a coin flip would take seven. The shape of the argument is unchanged by the roster being smaller than expected: the mode either runs on metagame knowledge or it does not run.
...
**What survives:** the global ban tier (§4, D5). A host saying "not tonight" is genuinely useful in casual play. It was called a _tournament_ ban because an organizer was assumed to set it; the host sets it, so it is now `globalBanned` throughout. The mechanism earned its place; the name did not.

**What this simplifies elsewhere:**

- `REPORT_RESULT` is single-sided with no `DISPUTE` state (D15). The two-sided-capable event schema recommended earlier is withdrawn — it existed only to survive a tournament retrofit that will not happen
- No match needs an owning entity above it. Room code plus seat token is the whole identity model, permanently, not provisionally
- The seat token being a bearer credential is acceptable rather than merely tolerable
