...
| O6  | **A 75-character roster makes the meta ban nearly blind.** `P(a named character sits among the opponent's draft)` is `draftCount/75` — about **5.3%** at 4. `bring-ban1`'s premise shifts from reading a draft to knowing an opponent's habits                                                            | Viable for casual repeat play, but it is a different mode than §9.2 describes. That UI copy is now wrong, and §15's meta-ban hit rate becomes the measure of whether the mode earns its place. Do **not** compensate by banning more — a coin-flip hit rate would take roughly 15 bans |
...
The roster is a versioned JSON asset (`roster/roster.json`), loaded and validated alongside the mode and snapshotted into the creation event. Nothing reads it at runtime. It currently holds **ten placeholder characters** so Phase 1 has something to draft against. The target is **~75**; see _On size_ below, because that number has design consequences.
...
**Too large and the ban goes blind.** The target roster is ~75. Under uniform drafting, the chance a named character sits among an opponent's 4 picks is `4/75 ≈ 5.3%` — a meta ban whiffs about nineteen times in twenty. See O6.
...
**Do not compensate with more bans.** Two bans move the hit rate from 5.3% to about 10.4%; reaching even a coin flip would take roughly 15. The mode either runs on metagame knowledge or it does not run.
