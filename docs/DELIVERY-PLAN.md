...
| **F7** | **Coverage cannot be measured through the Workers pool.** The v8 provider reports 0% across `apps/worker/src` while all 41 of its tests pass — instrumentation does not cross into workerd. Coverage thresholds are therefore scoped to `packages/*`, and the worker's assurance is its integration suite plus the two structural checks. Revisit if the pool gains an istanbul path                                                         |
